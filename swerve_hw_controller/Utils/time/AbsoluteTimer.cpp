...
bool AbsoluteTimer::wait(){
    ...
    // int m = read(_timerFd, &_missed, sizeof(_missed));
    // if(getSystemTime() - _startTime > _waitTimeUs + 150){
    //     std::cout << "[WARNING] The wait time " << _waitTimeUs << "us of AbsoluteTimer is not enough!" << std::endl
    //     << "The program has already cost " << getSystemTime() - _startTime << "us." << std::endl;
    //     return false;
    // }
    // return true;
}
